...
L2_BINARY_SEARCH_STEPS = 3
L2_BINARY_SEARCH_STEPS_1 = 3  # number of times to adjust the constant with binary search
...
class CarliniL2:
    def __init__(self, sess, model, image_size, num_channels, num_labels, batch_size=100,
                 ...
                 abort_early=L2_ABORT_EARLY, initial_const=L2_INITIAL_CONST):
        """
...
class CarliniFP:
    def __init__(self, sess, model, image_size, num_channels, num_labels, batch_size=100,
        ...
        # sum up the losses
        self.loss2 = tf.reduce_sum(self.l2dist)
        self.loss1 = tf.reduce_sum(self.const * (loss1 + loss_fp))
        self.loss = self.loss1 + self.loss2
        self.grads = tf.reduce_max(tf.gradients(self.loss, [modifier]))
