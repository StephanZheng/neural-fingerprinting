...
class LinfPGDAttack:
  def __init__(self, model, epsilon, k, a, random_start, loss_func, log_dir):
    ...
    if loss_func == 'xent':
      ...
      fixed_dxs = pickle.load(open(os.path.join(log_dir, "perturbations_dx.pkl"), "rb"))
      fixed_dys = pickle.load(open(os.path.join(log_dir, "perturbations_dy.pkl"), "rb"))
      for item, perturbs in enumerate(fixed_dxs):
          ...
          logits_p = model.model(model.x_input+perturbs)
          logits = model.logits
